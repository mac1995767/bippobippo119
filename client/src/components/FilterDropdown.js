  ...
  return (
    <div className="relative w-full p-4 bg-white rounded-lg shadow-md">
      ...
      <div className="flex justify-between items-center mb-4">
        <span className="text-gray-700 font-medium">예약 가능한 병원 보기</span>
        <label className="relative inline-flex items-center cursor-pointer">
          <input type="checkbox" className="sr-only peer" checked={isAvailable} onChange={toggleAvailability} />
          <div className="w-10 h-5 bg-gray-300 peer-focus:ring-4 peer-focus:ring-blue-300 rounded-full peer peer-checked:after:translate-x-full peer-checked:bg-blue-600 after:content-[''] after:absolute after:top-0.5 after:left-0.5 after:bg-white after:border after:border-gray-300 after:rounded-full after:h-4 after:w-4 after:transition-all"></div>
        </label>
      </div>
      ...
      <button
        ...
        className="w-full py-2 px-4 bg-gray-200 text-gray-700 font-medium rounded-md shadow hover:bg-gray-300"
      >
        필터 선택
      </button>

      {isOpen && (
        <div className="absolute top-full left-0 mt-2 w-full bg-white shadow-lg rounded-md p-4 z-10">
          {/* 카테고리 필터 */}
          ...
          {/* 병원 보기 버튼 */}
          <button className="w-full py-2 bg-pink-600 text-white font-semibold rounded-lg">
            검색 결과 보기
          </button>
        </div>
      )}
