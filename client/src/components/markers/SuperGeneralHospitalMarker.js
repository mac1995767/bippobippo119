...
const SuperGeneralHospitalMarker = ({ map, hospital, onClick, selected }) => {
  ...
  useEffect(() => {
    ...
    if (isNaN(lat) || isNaN(lng)) {
      console.warn('Invalid coords for SuperGeneralHospital:', hospital);
      return;
    }

    const position = new window.naver.maps.LatLng(lat, lng);
    const size = selected ? 24 : 16;
    const color = selected ? '#D32F2F' : '#EF5350';
...
export default SuperGeneralHospitalMarker; 