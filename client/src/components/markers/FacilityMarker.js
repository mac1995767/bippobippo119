...
const FacilityMarker = ({ map, facility, zoomLevel }) => {
  ...
  useEffect(() => {
    ...
    if (zoomLevel >= 12) {
        ...
        icon: {
          content: `
            <div style="background: white; padding: 5px; border-radius: 5px; box-shadow: 0 2px 6px rgba(0,0,0,0.3);">
              <div style="color: #0000FF; font-weight: bold;">${facility.name}</div>
              <div style="font-size: 12px; color: #666;">의료시설</div>
            </div>
          `,
      ...
      markerOptions = {
        ...markerOptions,
        icon: {
          content: '<div style="background: #0000FF; width: 10px; height: 10px; border-radius: 50%;"></div>',
