...
const GeneralHospitalMarker = ({ map, hospital, onClick, selected }) => {
  ...
  useEffect(() => {
    ...
    if (isNaN(lat) || isNaN(lng)) {
      console.warn('Invalid coords for GeneralHospital:', hospital);
      return;
    }

    const position = new window.naver.maps.LatLng(lat, lng);
    const size = selected ? 24 : 16;
    const color = selected ? '#E91E63' : '#F06292';
...
export default GeneralHospitalMarker; 